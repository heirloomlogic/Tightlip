...
- Property names may not be Swift keywords (`class`, `default`, …), member names Swift rejects (`Type`, `Protocol`, `_`), names the generated enum reserves for itself (`salt`, `decode`, and the symbols its decode shim references), or `envFile` — any of these would produce a non-compiling or ambiguous generated file, so the parser rejects them up front.
...
Every declared secret is required at build time. If an env var is unset, the build fails with one `error:` line per missing variable — all of them in the same build. An env var set to the empty string counts as set (with a `note:` in the log, since that's usually a leftover `export KEY=`). Truly optional values should be read from `ProcessInfo` at runtime rather than declared here.
...
- **More than two environments (qa, uat, etc.) or custom Xcode configuration names:** always set `TIGHTLIP_ENV` explicitly (per scheme for custom configurations).
...
If the configured file doesn't exist (typical on CI), the tool falls back to `ProcessInfo` only. Sourcing failures and timeouts (5s default) also fall back, with a single note to stderr; a file that errors *partway* still contributes the exports above the failing line, with a note that the capture may be partial.

The sourced file is tracked as a build input alongside `Secrets.yml`, so editing it re-triggers generation on the next build — no clean needed. (Values from non-file sources — your shell session, a scheme, a CI `env:` block — still need a clean build to pick up.)
...
Add a top-level `envFile:` directive before any secret declaration. The path is tilde-expanded against `$HOME`; relative paths resolve against the config's directory. It may not contain spaces or `#`, and a bare identifier (`envFile: SOME_VAR`) is rejected as ambiguous with a secret mapping — write `./SOME_VAR`.
...
**`error: environment variable X must be set to generate Secrets.Y`** — the env var is unset in both the sourced file and `ProcessInfo`. Every missing variable is reported in the same build. The accompanying `note:` line lists everything visible with the same prefix (e.g. `ACME_*`), which usually points at a typo. Confirm the key exists in your `envFile` (default `~/.zshenv`).

**`note: sourcing /path/to/file ... using process environment only`** — the subshell that sources your `envFile` failed, timed out, or exited before finishing. Reproduce with `zsh -f -c 'source <yourEnvFile>'`. Typical causes: a tool inside `.zshenv` (e.g. `mise`, `asdf`) hitting something the sandbox blocks, or `.zshenv` taking longer than 5 seconds.
...
**Plugin doesn't regenerate after changing an env var** — `Secrets.yml` and the sourced env file are tracked as build inputs, so edits to either re-trigger generation. Variables from anywhere else (shell session, scheme, CI `env:` block) aren't files the build system can watch — run `xcodebuild clean` (or Product → Clean Build Folder) and rebuild to pick those up.
