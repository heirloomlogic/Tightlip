...
/// Parses a HeirloomSecrets YAML config and any leading `envFile:` directive.
///
...
/// Namespace for HeirloomSecrets defaults to avoid polluting the importer's global scope.
public enum HeirloomSecretsDefaults {
    /// envFile path used when the YAML config does not specify one.
    public static let envFilePath = "~/.zshenv"
}

private let envFileHelperVar = "HEIRLOOM_SECRETS_ENV_FILE"
